...
Audit basis: the former public website reviewed during discovery  
Prepared for: UX/UI redesign and Figma planning  
Scope: unique layouts only. Repeated CMS entries, articles, categories, languages, and pagination URLs are grouped under reusable templates.
...
| Module detail | `/platform/{module}` | Six localized CMS entries using P14. |
...
| Insight category | `/insights/{category}` | Filterable result state; avoid indexing thin pagination combinations. |
...
| Sign in | `/sign-in` | Public authentication entry point. |

Use consistent localized route rules across English, German, French, and Italian. Maintain a redirect map from every valuable legacy URL; do not change slugs only for visual neatness.
...
```text
[OrgTik logo]  Platform ▾  Services ▾  Insights  Company ▾  Pricing
...
6. Sign in.
7. Language.
8. Full-width **Start a project** CTA.
...
| Evaluating one module | **Subscribe to this module** | Add to a custom plan |
...
The existing website, timeline/roadmap concept, new SaaS acquisition journey, and recommended proof system can be built from **17 unique page templates**:
...
- The two supplied timeline PDFs represent one page and one interaction state, not two page templates.
...
| 2 | Proof strip | Approved clients, Swiss location, verified systems delivered, support response, or uptime evidence. If proof is not verified, use client logos only. | Quiet horizontal band; monochrome logos; no animated counter carousel. | Optional **See selected work** |
...
| 8 | Infrastructure trust | Heading: **The infrastructure behind the experience.** Describe hosting, security, backups, monitoring, and support using only approved specifications. | Dense, technical dark band with one infrastructure diagram; no unsupported “50k+” or edge-node counters. | **Explore hosting** or contact |
...
Do not design each article. Design a robust CMS content system with reusable rich-text blocks.
...
Generate these groups from the CMS/navigation model so the page cannot drift out of date.

### Existing-site issue to solve

The current sitemap lists only one marketing entry and does not represent the website. The redesign should generate this page from the same navigation/content model used by the CMS.
...
Turn OrgTik’s company history, active work, future plans, and community suggestions into one traceable story. This is one new page template; the second supplied PDF is the **idea-detail drawer state**, not another page.
...
- Year navigation: 2021–2027. Years without supplied content remain CMS placeholders until approved milestones are provided.
...
The supplied second screen is a drawer/modal opened from a roadmap card. Its populated example is:
...
Dates, progress percentages, client names, performance results, roadmap commitments, votes, and comments in the supplied PDFs should be treated as **draft content** until OrgTik confirms them. Roadmap data needs an owner, update cadence, moderation policy, and a clear distinction between “planned” and a contractual promise.
...
1. Product header with Platform, Modules, Pricing, Sign in, and Contact Sales.
...
6. How setup works: choose → configure → sign in/subscribe → onboard.
...
10. CTA pair: **Subscribe to this module** and **Add to a custom plan**.
11. Footer.
...
5. **Continue securely** — preserve the configuration and route the visitor to P16 Sign in. If the customer is new, show the approved account-creation or sales-assisted path.
...
- Configuration saved in the URL/session before sign-in.
- Sign-in return restores the plan selection.
...
Add a persistent **Sign in** action to the desktop header and mobile drawer. Recommended public navigation:

```text
Home · Platform · Services · Insights · About
Pricing · Sign in · Contact Sales
```

“Platform” opens the SaaS overview/module navigation. “Services” keeps the existing agency/service business. “Sign in” must remain visually distinct from the primary commercial CTA.
...
Design only the public sign-in and recovery experience. The authenticated launcher, dashboard selector, individual dashboards, account settings, subscription management, and billing screens are not included in this website map.
...
|---|---|---|
| Brand film | 10–15 second desktop loop, mobile crop, poster, reduced-motion poster | Home and selected marketing heroes |
...
Background video is the primary hero treatment for OrgTik’s public marketing pages. The video supports the message; it must never reduce headline readability or become the only way information is communicated.

### Page-by-page video concepts

| Template | Background-video direction |
|---|---|
| P01 Home | A confident brand montage connecting Swiss business environments, product interfaces, infrastructure, and people at work. |
...
P06 Article detail, P09 Legal, P10 Sitemap, P11 Error, and P16 Sign in should default to static or poster-based heroes. They prioritize reading, recovery, or secure account access over cinematic presentation.
...
- Supply desktop and mobile crops, a poster image, captions/transcript only when the video contains meaningful spoken content, and descriptive alt/context in the CMS.
...
- Each CMS page needs: video asset, mobile asset, poster, focal point, overlay strength, motion-safe poster, and editorial approval status.
...
21 — Video Hero & Motion System
22 — Responsive Patterns
23 — Populated Content Examples
24 — Prototype Flows
```
...
Create **17 desktop master frames**, one for each template.
...
| 4 — Trust and participation | About, Timeline/Roadmap, proof/case-study modules | Establishes company credibility and innovation story. |
...
Hamburger
  → Navigation drawer
    → Platform / Pricing / Services / Insights / About / Contact / Sign in
...
Plans
  → Choose one module / bundle / custom workspace
    → Configure team and options
      → Review subscription
        → Sign in
          → Restore selection and continue to the approved subscription handoff
```

## Decisions required before high-fidelity design
...
11. Supply the logo assets, font source, background texture, hero videos/posters, product captures, and client media.
...
- **17 unique page templates.**
- **14 priority mobile layouts.**
...
The 120 discovered old-site routes are a content-model and CMS migration concern—not 120 separate design tasks. The supplied timeline references add one page master and one drawer state. The SaaS expansion adds four public/auth templates while deliberately excluding the six authenticated dashboards.
